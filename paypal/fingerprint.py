    ...
    p1_data = {
        ...
        "fp2": {
            "browser": {
                ...
                "timezoneOffset": 180,  # Brazil UTC-3
...
def send_otp_challenge(
    ...
    *,
    ...
    """Send the idapps/graphql getOtpChallengeOperation request.

    This is a PayPal risk-control prerequisite that must be called before
    InitiateRiskBasedTwoFactorPhoneConfirmationMutation.  The browser sends
    it with a csrfNonce and ctxId extracted from the signup page JavaScript.
    In protocol mode we generate compatible values if they are not available.
    """
    from loguru import logger
    import base64
    import secrets

    # Generate a csrfNonce if one was not extracted from the signup page.
    # The format observed in HAR captures is a base64url-ish token prefixed
    # with "AA".
    nonce = csrf_nonce or (
        "AA" + base64.urlsafe_b64encode(secrets.token_bytes(48)).decode("ascii").rstrip("=")
    )
    ...
    fn_data = build_signup_fn_sync_data(ec_token)

    # rData wraps fn_sync_data in an outer JSON object, then URL-encodes it.
    r_data_inner = json.dumps(
        {"fn_sync_data": fn_data},
        separators=(",", ":"),
    )
    r_data = urllib.parse.quote(r_data_inner, safe="")
    ...
    try:
        ...
        logger.info(
            "OTP challenge (idapps/graphql) HTTP {} bytes={}",
            resp.status_code,
            len(resp.content),
        )
    except Exception as e:
        logger.warning("OTP challenge request failed: {}", e)
        return

    # Try to extract csrfNonce / ctxId from response if it returns JSON
    try:
        result = resp.json()
        challenge = (
            result.get("data", {}).get("getOtpChallenge", {})
            if isinstance(result, dict)
            else {}
        )
        new_nonce = challenge.get("csrfNonce", "")
        new_ctx = challenge.get("ctxId", "")
        if new_nonce:
            session.state.csrf_nonce = new_nonce
        if new_ctx:
            session.state.ctx_id = new_ctx
