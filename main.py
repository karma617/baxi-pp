...
from paypal.models import generate_user, generate_card, generate_address, generate_country_materials
...
def main():
    ...
    parser.add_argument(
        ...
        choices=["BR", "US", "BA"],
        default="BR",
        help="PayPal flow country. Default: BR; US/BA enable their HAR-based flows.",
    )
    ...
    country = normalize_flow_country(args.country)
    if country != "BR":
        user, card, address, _profile = generate_country_materials(args.phone, country)
    else:
        user = generate_user(args.phone)
        card = generate_card(proxy_url=proxy_config.url)
        address = generate_address()
    flow_class = flow_class_for_country(country)
