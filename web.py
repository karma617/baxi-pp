...
class WebPayPalFlow(PayPalFlow):
    ...
    def refresh_proxy(self, exclude_urls: set[str] | None = None):
        """Re-resolve proxy for Phase0 DataDome retries.

        - api mode: fetch a brand-new dynamic proxy
        - pool mode: pick next unused entry from the same custom pool text
        """
        if not self.job.proxy_enabled:
            return None
        mode = (self.job.proxy_mode or "api").strip().lower()
        exclude_urls = set(exclude_urls or ())
        if mode == "pool":
            pool = parse_proxy_pool_text(self.job.proxy_pool_text)
            if not pool:
                return None
            cfg = build_proxy_config(enabled=True, pool=pool, exclude_urls=exclude_urls)
            if not cfg or not cfg.enabled or not cfg.entry:
                return None
            # Same masked host can still be the same sticky line; refuse reuse.
            if cfg.entry.url in exclude_urls:
                return None
            self.job._proxy_config = cfg
            self.job.proxy_label = cfg.label
            return cfg
        # API mode: try a few fresh pulls to avoid the same sticky exit.
        last_cfg = None
        for _ in range(3):
            cfg = build_dynamic_proxy_config(
                self.job.country,
                api_url=self.job.proxy_api_url or None,
            )
            last_cfg = cfg
            if cfg.entry and cfg.entry.url not in exclude_urls:
                self.job._proxy_config = cfg
                self.job.proxy_label = cfg.label
                return cfg
        if last_cfg and last_cfg.enabled:
            self.job._proxy_config = last_cfg
            self.job.proxy_label = last_cfg.label
            return last_cfg
        return None
    ...
    def _confirm_phone_with_retry(self, token: str, signup_url: str):
        """Web version of the CLI input loop with configured phone changes."""
        phone_changes = 0
        max_phone_changes = getattr(self, "max_phone_changes", 5)
        # Track browser assist per phone local number. Changing phone must allow
        # a fresh headed assist; otherwise HTTP re-initiate dies on authchallenge.
        ...
        def _initiate_with_optional_browser() -> tuple[str, str]:
            ...
            # After a phone change (or when HTTP is known challenged), prefer browser.
            if prefer_browser_initiate and phone_key not in browser_assist_phones:
                ...
                # Fall through to HTTP if browser did not return ids.
            ...
            try:
                ...
                if self._is_session_challenge_error(e):
                    # Let outer loop offer another phone change while quota remains.
                    raise
                raise
        ...
        while True:
            ...
            while True:
                ...
                # Normalize common OTP paste formats: spaces/dashes.
                compact = re.sub(r"[\s-]+", "", value or "")

                if (value or "").lower() in {"q", "quit", "exit"}:
...
def create_job(
    ...
    try:
        max_phone_changes = int(max_phone_changes)
    except Exception as exc:
        raise ValueError("最大换号次数必须是数字") from exc
    max_phone_changes = max(0, min(max_phone_changes, 20))
    debug = bool(debug) and ALLOW_DEBUG_LOGS
    mode = (proxy_mode or "api").strip().lower()
    if mode not in {"api", "pool"}:
        raise ValueError("代理模式只能是 api 或 pool")
    pool_text = (proxy_pool_text or "").strip()
    api_url = (proxy_api_url or "").strip()
    if bool(proxy_enabled):
        if mode == "pool" and not parse_proxy_pool_text(pool_text):
            # empty custom pool is allowed only if env/config fallback exists
            pass
        if mode == "api" and api_url and not (
            api_url.startswith("http://") or api_url.startswith("https://")
        ):
            raise ValueError("代理 API 地址必须以 http:// 或 https:// 开头")

    if bool(proxy_enabled):
        if mode == "pool":
            proxy_label = "任务启动时从代理池选择"
        else:
            proxy_label = "任务启动时从 API 获取"
    else:
        proxy_label = "代理关闭"
...
def run_job(job: WebJob) -> None:
    with logger.contextualize(job_id=job.id):
        ...
        try:
            ...
            job.set_status("running", "获取代理并生成资料")
            if job.proxy_enabled:
                mode = (job.proxy_mode or "api").strip().lower()
                if mode == "pool":
                    pool = parse_proxy_pool_text(job.proxy_pool_text)
                    if pool:
                        proxy_config = build_proxy_config(enabled=True, pool=pool)
                        logger.info("Proxy selected from custom pool: {}", proxy_config.label)
                    else:
                        proxy_config = build_proxy_config(enabled=True)
                        logger.info(
                            "Custom proxy pool empty; using env/config pool: {}",
                            proxy_config.label,
                        )
                else:
                    try:
                        proxy_config = build_dynamic_proxy_config(
                            job.country,
                            api_url=job.proxy_api_url or None,
                        )
                        logger.info(
                            "Dynamic proxy fetched for {} via {}: {}",
                            job.country,
                            "custom API" if job.proxy_api_url else "default API",
                            proxy_config.label,
                        )
                    except Exception as proxy_exc:
                        logger.warning(
                            "Dynamic proxy API failed ({}), falling back to local/env proxy pool",
                            proxy_exc,
                        )
                        proxy_config = build_proxy_config(enabled=True)
            else:
            ...
            flow_class = flow_class_for_country(job.country)
            ...
            logger.info("Country: {}", job.country)
            ...
            flow = type("Web" + flow_class.__name__, (WebPayPalFlow, flow_class), {})(
                ba_token=job.ba_token,
                user=user,
                card=card,
                address=address,
                max_card_attempts=job.max_card_attempts,
                max_phone_changes=job.max_phone_changes,
...
class WebHandler(BaseHTTPRequestHandler):
    ...
    def serve_static(self, path: str) -> None:
        ...
        content_type = self.static_content_type(resolved)
